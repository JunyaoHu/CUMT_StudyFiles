...
#     for f in fx:
#         if f < MIN_OUTPUT or f > MAX_OUTPUT:
#             f = 0
#     fx = -sum(fx)
...
#             xNew[v] = xNow[v] + 0.5 * scale * random.randint(xMin[v] - int(xNow[v]), xMax[v] - int(xNow[v]))
#             # v = random.randint(0, nVar - 1)  # 产生 [0,nVar-1]之间的随机数
#             # fiveSum = 0
#             # for i in range(nVar):
#             #     if i != v:
...
# xMin = [MIN_QUANTITY] * nVar  # 给定搜索空间的下限，x1_min,..xn_min
# xMax = [MAX_QUANTITY - 1] * nVar  # 给定搜索空间的上限，x1_max,..xn_max
...
class PSO:
    def __init__(self, N=20, iter_N=100):
        ...
        self.c1 = 1  # 自我认知学习因子
        self.c2 = 2  # 社会认知学习因子
        self.gbest = [500] * 6  # 种群当前最好位置
        self.N = N  # 种群中粒子数量
        self.POP = []  # 种群
        self.iter_N = iter_N  # 迭代次数

    # 适应度值计算函数
    def fitness(self, x, mk):
        # p01 = (max(0, -x[0] + MIN_QUANTITY)) ** 2
        # p02 = (max(0, -x[1] + MIN_QUANTITY)) ** 2
        # p03 = (max(0, -x[2] + MIN_QUANTITY)) ** 2
        # p04 = (max(0, -x[3] + MIN_QUANTITY)) ** 2
    ...
    # 找到全局最优解
    def g_best(self, pop, i):
        for bird in pop:
            if bird.fitness > self.fitness(self.gbest, i):
                self.gbest = bird.p
    ...
    def initPopulation(self, pop, N):
        for i in range(N):
            bird = Particle()
            bird.p = [np.random.randint(MIN_QUANTITY, MAX_QUANTITY - 1)] * 6
            bird.fitness = self.fitness(bird.p, i)
    ...
    def update(self, pop, i):
        for bird in pop:
            ...
            if -10 < p < 10:
                ...
                # 是否需要更新本粒子历史最好位置
                if bird.fitness > self.fitness(bird.pbest, i):
                    bird.pbest = bird.p
...
print("最优解 x = ", pso.gbest, "相应最大值 f(x) = ", -pso.fitness(pso.gbest, 0))
