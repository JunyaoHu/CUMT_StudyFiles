...
Private Sub Ok3w_Article_PreNext01(ClassID,ID)
	...
	If oRs.Eof And oRs.bof Then
		...
		Else
			Response.Write(Format_TitleURL(HTML_Article_Path,oRs("ID"),"&nbsp;<strong>前一条</strong>：" & oRs("Title"),oRs("TitleColor"),oRs("TitleURL"),50))
	End If
	Response.Write("</td>")
	Response.Write("</tr>")
	...
	If oRs.Eof And oRs.bof Then
		...
		Else
			Response.Write(Format_TitleURL(HTML_Article_Path,oRs("ID"),"&nbsp;<strong>后一条</strong>：" & oRs("Title"),oRs("TitleColor"),oRs("TitleURL"),50))
	End If
	Response.Write("</td>")
	Response.Write("</tr>")
...
Private Sub Ok3w_Article_bList01(ClassID,PageSize)
	Sql="select Id,Title,TitleColor,TitleURL,Description,Author,PicFile,Hits,ComeFrom,AddTime from Ok3w_Article where IsPass=1 and IsDelete=0 and SortPath like '%," & ClassID & ",%' order by AddTime desc,ID desc"
	Call Page.GetRs(Conn,Rs,Sql,PageSize)
	oCount = 0
	Response.Write("<div class=""box2""><ul>")
	Do While Not Rs.Eof And Not Page.Eof
		...
		If Rs("TitleURL")="" Then
			...
			Else
				TitleURL = Rs("TitleURL")
			End If
		Response.Write("<li><div class=hangju02 style=""FLOAT:left""><A href=""" & TitleURL & """ target=_blank><div id=div1><IMG height=""110px"" src=""" & Rs("PicFile") & """ width=""147px""></div></A></div><span class=""dateR"">" & Format_Time(Rs("AddTime"),2) & "</span>" & Format_TitleURL(HTML_Article_Path,Rs("ID"),Rs("Title"),Rs("TitleColor"),Rs("TitleURL"),50) &"")
		Response.Write("<br/><span class=""hangju03"">"& Rs("Description")&"</span>")
		Response.Write("<br/><span class=""hangju01"">"& Rs("Author")&"&nbsp;|&nbsp;"& Rs("ComeFrom")&"&nbsp;|&nbsp;"& Rs("Hits")&"次阅读</span></li>")
     	If oCount Mod 5 = 0 And oCount < PageSize Then
			Response.Write("<li class=sep></li>")
		End If
		Rs.MoveNext
...
Private Sub Ok3w_Article_IndexClassImg01(ClassID,TopN,WW,HH)
	Sql="select top " & TopN & " Id,Title,TitleColor,TitleURL,Description,Hits,ComeFrom,AddTime,Author,PicFile from Ok3w_Article where ChannelID=1 and IsPass=1 and IsDelete=0 and IsIndexImg=1"
	If ClassID<>"" Then Sql=Sql & " and SortPath like '%," & ClassID & ",%'"
	Sql=Sql & " order by IsTop desc,IsCommend desc,AddTime desc,Id desc"
	Rs.Open Sql,Conn,0,1
	...
	Else
		Do While Not Rs.Eof
			...
			Response.Write("<div class=""summary001"">")
			Response.Write("<div class=imgDiv001 style=""FLOAT:left""><A href=""" & TitleURL & """ target=_blank><div id=div1><IMG height=""" & HH & """ src=""" & PicFile & """ width=""" & WW & """ alt=""" & Author & """></div></A></div>")
            Response.Write("<div class=cont>")
           	Response.Write("<div class=title><A class=nLink001 href=""" & TitleURL & """ target=_blank>" & Title & "</A></div>")
            Response.Write("<div class=sum>" & Description & "</div>")
			Response.Write("<div class=sum001>" & AddTime & "&nbsp;|&nbsp;"& ComeFrom & "&nbsp;|&nbsp;"& Author & "&nbsp;|&nbsp;" & Hits & "次阅读</div>")
            Response.Write("</div>")
			Response.Write("</div>")
			Rs.MoveNext
		Loop
...
Private Sub Ok3w_Article_pList(ClassID,TopN,PageSize,Cels,ImgW,ImgH,CharX)
	...
	Else
		...
		Response.Write("<img src=""" & Rs("PicFile") & """ alt=""" & Rs("Title") & """ width=""" & ImgW & """ height=""" & ImgH & """ border=""0""  class=""canyin003""/>")
...
Private Sub Ok3w_Article_IndexClassImg(ClassID,TopN,WW,HH)
	...
	Else
		Do While Not Rs.Eof
			...
			PicFile = Rs("PicFile")
			Description = Rs("Description")
			Response.Write("<div class=imgDiv style=""FLOAT: left""><A class=imgLink href=""" & TitleURL & """target=""_blank""><IMG height=""" & HH & """ src=""" & PicFile & """ width=""" & WW & """></A></div>")
            Response.Write("<div class=cont>")
           	Response.Write("    <div class=title><A class=nLink href=""" & TitleURL & """target=""_blank"">" & Title & "</A></div>")
            Response.Write("	<div class=sum>" & Description & "</div>")
            Response.Write("</div>")
			Rs.MoveNext
		Loop
...
Private Sub Ok3w_Article_IndexTop()
	...
	If Rs.Eof And Rs.Bof Then
	Else
		Response.Write("<A href=""" & Page_URL(ChannelID, "",Rs("ID")) & """target=""_blank"">" & Rs("Title") & "</A>")
	End If
	Rs.Close
End Sub
...
Rs.Open Sql,Conn,0,1
Do While Not Rs.Eof
	StrTmp = StrTmp & "<a href=""" & Rs("Lurl") & """ target=""_blank""><img src=""" & Rs("Lpic") & """ alt=""" & Rs("Lname") & """ width=""160"" height=""60"" border=""0"" class=gr004 /></a> "
	Rs.MoveNext
Loop
Rs.Close
End If
...
Private Sub Ok3w_Article_IndexClassImgwap(ClassID,TopN,WW,HH)
	Sql="select top " & TopN & " Id,Title,TitleColor,TitleURL,Description,Hits,ComeFrom,AddTime,Author,PicFile from Ok3w_Article where ChannelID=1 and IsPass=1 and IsDelete=0 and IsIndexImg=1"
	If ClassID<>"" Then Sql=Sql & " and SortPath like '%," & ClassID & ",%'"
	Sql=Sql & " order by IsTop desc,IsCommend desc,AddTime desc,Id desc"
	Rs.Open Sql,Conn,0,1
	...
	Else
		Do While Not Rs.Eof
			...
			Description = Rs("Description")
			Response.Write("<div class=""summary001"">")
			Response.Write("<div class=imgDiv001 style=""FLOAT:left""><A href=""" & TitleURL & """ target=_blank><div id=div1><IMG height=""" & HH & """ src=""" & PicFile & """ width=""" & WW & """></div></A></div>")
            Response.Write("<div class=cont>")
           	Response.Write("<div class=title><A class=nLink001 href=""" & TitleURL & """ target=_blank>" & Title & "</A></div>")
			Response.Write("<div class=sum001>" & Hits & "次阅读</div>")
            Response.Write("</div>")
			Response.Write("</div>")
			Rs.MoveNext
		Loop
...
Private Sub Ok3w_Article_IndexClassImg02(ClassID,TopN,WW,HH)
	Sql="select top " & TopN & " Id,Title,TitleColor,TitleURL,Description,Hits,ComeFrom,AddTime,PicFile from Ok3w_Article where ChannelID=1 and IsPass=1 and IsDelete=0 and IsIndexImg=1"
	If ClassID<>"" Then Sql=Sql & " and SortPath like '%," & ClassID & ",%'"
	Sql=Sql & " order by IsTop desc,IsCommend desc,AddTime desc,Id desc"
	Rs.Open Sql,Conn,0,1
	If Rs.Eof And Rs.Bof Then
	Else
		Do While Not Rs.Eof
			...
			Response.Write("<div class=imgDiv002 style=""FLOAT:left""><A href=""" & TitleURL & """ target=_blank><IMG height=""" & HH & """ src=""" & PicFile & """ width=""" & WW & """></A></div>")
			...
			Response.Write("<div class=sum002>" & AddTime & "</div>")
            Response.Write("</div>")
			Response.Write("</div>")
			Rs.MoveNext
		Loop
...
Private Sub Ok3w_Article_IndexClassImg03(ClassID,TopN,WW,HH)
	Sql="select top " & TopN & " Id,Title,TitleColor,TitleURL,Description,Hits,ComeFrom,AddTime,PicFile from Ok3w_Article where ChannelID=1 and IsPass=1 and IsDelete=0 and IsIndexImg=1"
	If ClassID<>"" Then Sql=Sql & " and SortPath like '%," & ClassID & ",%'"
	Sql=Sql & " order by IsTop desc,IsCommend desc,AddTime desc,Id desc"
	Rs.Open Sql,Conn,0,1
	...
	Else
		Do While Not Rs.Eof
            ...
            Response.Write("<div class=cont002>")
           	Response.Write("<div class=title002><A class=nLink002 href=""" & TitleURL & """ target=_blank>" & Title & "</A></div>")
            Response.Write("<div class=sum002>活动热度：" & Hits & "</div>")
			Response.Write("<div class=sum002>" & AddTime & "</div>")
            Response.Write("</div>")
			Response.Write("</div>")
			Rs.MoveNext
