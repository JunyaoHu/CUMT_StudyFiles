...
Class Ok3w_Article
    ...
    Private Sub GetFormData()
		...
		wapshipin = Request.Form("wapshipin")
		Description = Request.Form("Description")
		neibujia = Request.Form("neibujia")
    ...
    Private Sub GetRs(ByRef Rs)
		...
		wapshipin = Rs("wapshipin")
        Description = Rs("Description")
        Content = Rs("Content")
        Author = Rs("Author")
        ComeFrom = Rs("ComeFrom")

        if isnull(Rs("AddTime")) then 
        	AddTime = Now()
        else
        	AddTime = Rs("AddTime")
        end if
	...
	Public Sub User_Article_Del(a_ID,User_Name)
		...
		Sql = "delete from Ok3w_Article where ID=" & Cdbl(a_ID) & " and Inputer='" & User_Name & "'"
		Conn.Execute Sql
	End Sub
End Class
%>
