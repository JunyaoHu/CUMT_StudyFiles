...
Class zhuangao
	...
	Public Sub Load(ID)
		...
		Sql = "select * from zhuangao where ID=" & ID
	...
	Public Sub UserEdit()
		...
		Sql="select count(ID) from zhuangao where Mail='" & Mail & "' and User_Name<>'" & User_Name & "'"
		If Conn.Execute(Sql)(0)<>0 Then
			Call MessageBox("该邮箱已经存在，请重新输入","")
		Else
			Sql = "select * from zhuangao where User_Name='" & User_Name & "' and User_Password='" & MD5(User_Password) & "'"
			Rs.Open Sql,Conn,1,3
			If Rs.Eof And Rs.Bof Then
	...
	Public Sub Pass(action,ID)
		If ID <> "" Then
			sql = "update zhuangao set IsLock=" & action & " where ID in(" & ID & ")"
			Conn.Execute sql
		End If
	End Sub
	...
	Public Sub Del(ID)
		If ID <> "" Then
			Sql="delete from zhuangao where ID in(" & ID & ")"
			Conn.Execute Sql
		End If
	End Sub	
	...
	Public Function Login(sName,sPwd)
		Sql = "select * from zhuangao where User_Name=? and User_Password=?"
				...
				If clsRs("IsLock") = 1 Then
					...
					Else
						...
						If DateDiff("d",clsRs("LastLoginTime"),Date())=0 Then
							Sql="update zhuangao set LastLoginIp='" & Request.ServerVariables("REMOTE_ADDR") & "',LastLoginTime=" & Db_DateTime & ",LoginCount=LoginCount+1 where ID=" & clsRs("ID")
						Else
							Sql="update zhuangao set LastLoginIp='" & Request.ServerVariables("REMOTE_ADDR") & "',LastLoginTime=" & Db_DateTime & ",LoginCount=LoginCount+1,Jifen=Jifen+" & Application(SiteID & "_Ok3w_SiteUserLoginJifen") & " where ID=" & clsRs("ID")
						End If
						Conn.Execute Sql
    ...
    Public Function GetMaxUserID()
        Set clsRs = Server.CreateObject("Adodb.RecordSet")
        Sql = "select max(ID) from zhuangao"
