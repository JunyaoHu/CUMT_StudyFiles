...
Class UpLoadClass
	...
	Private Sub Class_Initialize
		...
		m_Charset  = "gb2312"
		m_TotalSize= 0
		m_MaxSize  = 15360000000
		m_FileType = ""
		m_SavePath = "pdf/"
		
	if stu_name<>"" then
		m_SavePath = "pdf/"&trim(stu_name)&"/"	
	set fso=server.CreateObject("Scripting.FileSystemObject")
	if not fso.folderExists(server.MapPath(m_SavePath)) then
		fso.Createfolder(server.MapPath(m_SavePath))
	end if
	set fso=nothing
		end if
		
	if college<>"" then	
		m_SavePath = m_SavePath &trim(college)&"/"
	set fso=server.CreateObject("Scripting.FileSystemObject")
	if not fso.folderExists(server.MapPath(m_SavePath)) then
		fso.Createfolder(server.MapPath(m_SavePath))
	end if
	...
	if qianzheng<>"" then	
		m_SavePath = m_SavePath &"qianzheng/"
	set fso=server.CreateObject("Scripting.FileSystemObject")
	if not fso.folderExists(server.MapPath(m_SavePath)) then
		fso.Createfolder(server.MapPath(m_SavePath))
	end if
