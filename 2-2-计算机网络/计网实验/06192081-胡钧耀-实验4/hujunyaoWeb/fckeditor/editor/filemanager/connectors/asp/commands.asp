...
Sub FileUpload( resourceType, currentFolder, sCommand )
	...
	Else
		...
		if not (oFSO.FolderExists( sServerDir ) ) then
			...
			sFileName = GetRndFileName(sExtension)'SanitizeFileName( sFileName )
			...
			Do While ( True )
				...
				Else
					...
					If sExtension = "gif" Or sExtension = "jpg" Or sExtension = "jpeg" Or sExtension = "png" Or sExtension = "bmp" Then
						Call Ok3w_AspJpeg(sFilePath)
					End If
...
Function GetRndFileName(sExt)
	Dim oDateTime
	oDateTime = Now()
	GetRndFileName = Year(oDateTime) & Get2BitChar(Month(oDateTime)) & Get2BitChar(Day(oDateTime)) & Get2BitChar(Hour(oDateTime)) & Get2BitChar(Minute(oDateTime)) & Get2BitChar(Second(oDateTime)) & "." & sExt
End Function

Function Get2BitChar(Str)
	Get2BitChar = Right("00" & Str,2)
End Function
%>
