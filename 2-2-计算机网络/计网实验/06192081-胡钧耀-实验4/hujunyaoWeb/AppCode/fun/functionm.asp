...
'区域名称
Function get_area(ClassId)
	Sql = "select SortName from Ok3w_area where ID=" & ClassId
	get_area = ExecSqlReturnOneValue(Sql)
End Function

'经销商名称
Function get_agency(ClassId)
	Sql = "select [names] from agency where ID=" & ClassId
	get_agency = ExecSqlReturnOneValue(Sql)
End Function

'酒名
Function get_goods_name(ClassId)
	Sql = "select [pro_name] from Ok3w_Article where ID=" & ClassId
	get_goods_name = ExecSqlReturnOneValue(Sql)
End Function
'经销商促销数量
Function get_goods_num(ClassID)
	Sql="select count(ID) from agency_cx where agency_id=" & ClassID
	get_goods_num=Conn.Execute(Sql)(0)
End Function

'经销商促销数量
Function get_agency_num(ClassID)
	Sql="select count(ID) from agency_cx where pro_id=" & ClassID
	get_agency_num=Conn.Execute(Sql)(0)
End Function
...
'栏目名称
Function GetAreaName(ClassId)
	If ClassId="0" Then
		GetAreaName = "--------"
		Exit Function
	End If
	Sql = "select SortName from Ok3w_area where ID=" & ClassId
	GetAreaName = ExecSqlReturnOneValue(Sql)
End Function
...
'		OutContent = Mid(Content,(thisPage-1)*2000+1,2000)
	...
	OutContent = ReplaceTest(OutContent,"<img ","<img style=""cursor:pointer;"" onclick=""openURL(this.src)"" onload=""zoomIMG(this,800)"" ")
